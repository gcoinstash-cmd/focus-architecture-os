...
export const AdminPortalModal: React.FC<AdminPortalModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/85 backdrop-blur-xl">
      <div className="relative w-full max-w-4xl bg-[#08080A] border border-[#C5A85C]/40 shadow-[0_0_50px_rgba(197,168,92,0.2)] rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Terminal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black/90">
          <div className="flex items-center gap-3">
            <span className="w-2.5 h-2.5 bg-[#C5A85C] rounded-full animate-ping" />
            <span className="font-mono text-xs uppercase tracking-[0.25em] text-[#C5A85C] font-bold flex items-center gap-2">
              <Terminal size={14} /> FOCUS_ARCHITECTURE_OS // EXECUTIVE_BYPASS
            </span>
          </div>
          <button 
            onClick={onClose}
        ...
        {!isAuthenticated ? (
          <div className="p-8 sm:p-12 flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#C5A85C]/10 border border-[#C5A85C]/30 flex items-center justify-center text-[#C5A85C] mb-6 shadow-[0_0_30px_rgba(197,168,92,0.15)]">
              <Lock size={28} />
            </div>
            ...
            <form onSubmit={handleLogin} className="w-full max-w-sm space-y-4">
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-black/60 border border-white/15 focus:border-[#C5A85C] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-[#C5A85C] font-mono text-center tracking-widest"
                />
              </div>
          ...
          <div className="p-6 space-y-4 font-mono text-xs overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <span className="text-[#C5A85C] font-bold flex items-center gap-2">
                <Workflow size={14} /> EXECUTIVE FOCUS CHAMBER // VERIFIED
              </span>
